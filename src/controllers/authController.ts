...
export class AuthController {
  ...
  static logoutOtherDevices = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    if (!req.user || !req.sessionId) {
      return next(new AppError("Not logged in", 401))
    }
  ...
  static logoutAllDevices = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    if (!req.user) {
      return next(new AppError("Not logged in", 401))
    }
  ...
  static socialAuthCallback = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { accessToken, refreshToken } = await AuthService.generateTokensForUser(req.currentUser.id, req.sessionId)
