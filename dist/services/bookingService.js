...
class BookingService {
    static async createBooking(userId, data) {
        try {
            ...
            await email_1.EmailService.sendBookingConfirmation(booking.user.email, booking);
            ...
            logger_1.logger.error(`Error in createBooking: ${error}`);
    ...
    static async getBookings(userId, params) {
        try {
            const { page = 1, limit = 10, sortBy = "created_at", sortOrder = "desc", status, booking_type, startDate, endDate, } = params;
            ...
            const totalCount = await prisma_1.prisma.booking.count({
                where: { user_id: userId },
            });
            // Get filtered count
            const filteredCount = await prisma_1.prisma.booking.count({ where });
            // Get paginated data
            ...
            logger_1.logger.error(`Error in getBookings: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get bookings", 500);
        }
    }
    static async getBookingById(userId, id) {
        try {
            ...
            if (booking.user_id !== userId) {
                throw new appError_1.AppError("You are not authorized to view this booking", 403);
            }
            ...
            logger_1.logger.error(`Error in getBookingById: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get booking", 500);
        }
    }
    static async updateBooking(userId, id, data) {
        try {
            const booking = await prisma_1.prisma.booking.findUnique({
                where: { id },
            });
            ...
            if (booking.status === "COMPLETED" || booking.status === "CANCELED") {
                throw new appError_1.AppError(`Cannot update a ${booking.status.toLowerCase()} booking`, 400);
            }
            // If changing dates, validate availability
            if ((data.start_date || data.end_date) && booking.booking_type === "HOTEL" && booking.hotel_id) {
                await this.validateHotelAvailability(booking.hotel_id, data.start_date || booking.start_date.toISOString(), data.end_date || booking.end_date.toISOString());
            }
            else if ((data.start_date || data.end_date) && booking.booking_type === "TRIP" && booking.trip_id) {
                await this.validateTripAvailability(booking.trip_id, data.start_date || booking.start_date.toISOString(), data.end_date || booking.end_date.toISOString());
            }
            ...
            if ((data.start_date || data.end_date || data.guests) && booking.booking_type === "HOTEL" && booking.hotel_id) {
                totalPrice = await this.calculateHotelPrice(booking.hotel_id, data.start_date || booking.start_date.toISOString(), data.end_date || booking.end_date.toISOString(), data.guests || booking.guests || 0);
            }
            ...
            if (data.status) {
                await email_1.EmailService.sendBookingStatusUpdate(updatedBooking.user.email, updatedBooking);
                if (updatedBooking.user.phone_number) {
                    await sms_1.SmsService.sendBookingStatusUpdateSms(updatedBooking.user.phone_number, updatedBooking.id);
                }
            }
            return updatedBooking;
        }
        catch (error) {
            logger_1.logger.error(`Error in updateBooking: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to update booking", 500);
        }
    }
    static async cancelBooking(userId, id) {
        try {
            const booking = await prisma_1.prisma.booking.findUnique({
                where: { id },
                include: {
            ...
            const canceledBooking = await prisma_1.prisma.booking.update({
                where: { id },
                data: {
                    status: "CANCELED",
                },
            ...
            await email_1.EmailService.sendBookingCancellation(booking.user.email, booking);
            if (booking.user.phone_number) {
                await sms_1.SmsService.sendBookingCancellationSms(booking.user.phone_number, booking.id);
            }
            return {
            ...
            logger_1.logger.error(`Error in cancelBooking: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to cancel booking", 500);
        }
    }
    static async validateHotelAvailability(hotelId, startDate, endDate) {
        ...
        if (availableRooms <= 0) {
            throw new appError_1.AppError("Hotel is fully booked for the selected dates", 400);
        }
        return true;
    }
    static async validateFlightAvailability(flightId) {
        ...
        if (!flight) {
            throw new appError_1.AppError("Flight not found", 404);
        }
        ...
        if (availableSeats <= 0) {
            throw new appError_1.AppError("Flight is fully booked", 400);
        }
        return true;
    }
    static async validateTripAvailability(tripId, startDate, endDate) {
        ...
        if (availableSpots <= 0) {
            throw new appError_1.AppError("Trip is fully booked for the selected dates", 400);
        }
        return true;
    }
    static async calculateHotelPrice(hotelId, startDate, endDate, guests) {
        const hotel = await prisma_1.prisma.hotel.findUnique({
            where: { id: hotelId },
            select: {
    ...
    static async calculateFlightPrice(flightId, passengers) {
        const flight = await prisma_1.prisma.flight.findUnique({
            where: { id: flightId },
            select: {
                price: true,
    ...
    static async calculateTripPrice(tripId, participants) {
        const trip = await prisma_1.prisma.trip.findUnique({
            where: { id: tripId },
            select: {
                price: true,
