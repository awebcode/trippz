...
class HotelService {
    static async createHotel(data) {
        try {
            ...
            logger_1.logger.error(`Error in createHotel: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to create hotel", 500);
        }
    }
    static async getHotels(params) {
        try {
            const { page = 1, limit = 10, sortBy = "created_at", sortOrder = "desc", location, city, country, check_in, check_out, guests, rooms, minPrice, maxPrice, rating, amenities, distance_from_center, has_free_cancellation, has_breakfast_included, has_parking, has_pool, has_gym, has_restaurant, has_room_service, has_spa, has_wifi, has_air_conditioning, is_pet_friendly, is_family_friendly, } = params;
            ...
            amenityFilters.forEach((filter) => {
                if (filter.value !== undefined) {
                    ...
                    if (filter.value === true) {
                        ...
                        where.amenities.hasSome.push(filter.field.replace("has_", "").replace("is_", ""));
                    }
                }
            });
            if (guests !== undefined) {
            ...
            const totalCount = await prisma_1.prisma.hotel.count();
            // Get filtered count
            const filteredCount = await prisma_1.prisma.hotel.count({ where });
            // Get paginated data
            ...
            logger_1.logger.error(`Error in getHotels: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get hotels", 500);
        }
    }
    static async getHotelById(id) {
        try {
            ...
            logger_1.logger.error(`Error in getHotelById: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get hotel", 500);
        }
    }
    static async updateHotel(id, data) {
        try {
            ...
            const updatedHotel = await prisma_1.prisma.hotel.update({
                where: { id },
                data: {
                    name: data.name,
                    address: data.address,
            ...
            logger_1.logger.error(`Error in updateHotel: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to update hotel", 500);
        }
    }
    static async deleteHotel(id) {
        try {
            ...
            if (activeBookings > 0) {
                throw new appError_1.AppError("Cannot delete hotel with active bookings", 400);
            }
            // Delete hotel images
            ...
            logger_1.logger.error(`Error in deleteHotel: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to delete hotel", 500);
        }
    }
    static async searchHotels(params) {
        try {
            ...
            logger_1.logger.error(`Error in searchHotels: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to search hotels", 500);
        }
    }
    static async getHotelAvailability(params) {
        try {
            ...
            logger_1.logger.error(`Error in getHotelAvailability: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to check hotel availability", 500);
        }
    }
    // Custom query method for advanced hotel searches
    static async customHotelQuery(params) {
        try {
            const { page = 1, limit = 10, sortBy = "created_at", sortOrder = "desc", 
            // Custom query parameters
            nearby_attractions, nearby_landmarks, nearby_restaurants, nearby_shopping, nearby_public_transport, distance_radius, property_type, star_rating, guest_rating_min, room_types, ...rest } = params;
            // Calculate pagination values
            const skip = (page - 1) * limit;
            ...
            if (guest_rating_min) {
                ...
                logger_1.logger.info(`Filtering for minimum guest rating: ${guest_rating_min}`);
            }
            // Add room types filter
            if (room_types && room_types.length > 0) {
                // This would typically be a join to a room_types table
                // For now, we'll log it as a placeholder
                logger_1.logger.info(`Filtering for room types: ${room_types.join(", ")}`);
            }
            // Get filtered count
            const filteredCount = await prisma_1.prisma.hotel.count({ where });
            // Get paginated data
            ...
            if (guest_rating_min) {
                processedHotels = processedHotels.filter((hotel) => hotel.avg_rating >= guest_rating_min);
            }
            // Calculate total pages
            const totalPages = Math.ceil(filteredCount / limit);
            return {
                data: processedHotels,
                metadata: {
                    totalCount: await prisma_1.prisma.hotel.count(),
            ...
            logger_1.logger.error(`Error in customHotelQuery: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to execute custom hotel query", 500);
        }
    }
}
