...
class DestinationService {
        ...
        try {
            ...
            if (files && files.length > 0) {
                await Promise.all(files.map(async (file, index) => {
                    const uploadResult = await (0, fileUpload_1.uploadToCloudinary)(file.path, "trippz/destinations");
            ...
            logger_1.logger.error(`Error in createDestination: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to create destination", 500);
        }
    }
    static async getDestinations(query = {}) {
        try {
            ...
            // Get total count
            const totalCount = await prisma_1.prisma.destination.count({ where });
            ...
            logger_1.logger.error(`Error in getDestinations: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get destinations", 500);
        }
    }
    static async getDestinationById(id) {
        try {
            ...
            if (!destination) {
                throw new appError_1.AppError("Destination not found", 404);
            }
            return destination;
        }
        catch (error) {
            logger_1.logger.error(`Error in getDestinationById: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get destination", 500);
        }
    }
    static async updateDestination(id, data, files) {
        try {
            ...
            const updatedDestination = await prisma_1.prisma.destination.update({
                where: { id },
                data: {
                    name: data.name,
                    country: data.country,
            ...
            logger_1.logger.error(`Error in updateDestination: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to update destination", 500);
        }
    }
    static async deleteDestination(id) {
        try {
            ...
            if (destination.packages.length > 0) {
                throw new appError_1.AppError("Cannot delete destination that is used in travel packages", 400);
            }
            ...
            logger_1.logger.error(`Error in deleteDestination: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to delete destination", 500);
        }
    }
    static async setFeaturedImage(id, imageId) {
        try {
            const destination = await prisma_1.prisma.destination.findUnique({
                where: { id },
                include: {
            ...
            if (!image) {
                throw new appError_1.AppError("Image not found for this destination", 404);
            }
            ...
            logger_1.logger.error(`Error in setFeaturedImage: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to set featured image", 500);
        }
    }
    static async getTrendingDestinations(limit = 5) {
        try {
            ...
            const trendingDestinations = await prisma_1.prisma.$queryRaw `
      ...
      `;
            ...
            logger_1.logger.error(`Error in getTrendingDestinations: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get trending destinations", 500);
        }
    }
    static async getNearbyDestinations(latitude, longitude, radiusKm = 100, limit = 5) {
        try {
            // Calculate nearby destinations using Haversine formula
            const nearbyDestinations = await prisma_1.prisma.$queryRaw `
        SELECT *, 
      ...
      `;
            ...
            logger_1.logger.error(`Error in getNearbyDestinations: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get nearby destinations", 500);
        }
    }
    static async getDestinationWeather(id) {
        try {
            const destination = await prisma_1.prisma.destination.findUnique({
                where: { id },
            ...
            if (!destination.latitude || !destination.longitude) {
                throw new appError_1.AppError("Destination coordinates not available", 400);
            }
            ...
            logger_1.logger.error(`Error in getDestinationWeather: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get destination weather", 500);
        }
    }
    static async getDestinationAttractions(id, limit = 10) {
        try {
            const destination = await prisma_1.prisma.destination.findUnique({
                where: { id },
            });
            ...
            logger_1.logger.error(`Error in getDestinationAttractions: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get destination attractions", 500);
        }
    }
}
