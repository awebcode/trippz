...
class FlightService {
    static async createFlight(data) {
        try {
            ...
            logger_1.logger.error(`Error in createFlight: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to create flight", 500);
        }
    }
    static async getFlights(params) {
        try {
            const { page = 1, limit = 10, sortBy = "departure_time", sortOrder = "asc", from, to, departure_date, return_date, seat_class, minPrice, maxPrice, airline, direct_flights_only, has_wifi, has_entertainment, has_power_outlets, meal_service, min_baggage_allowance, } = params;
            ...
            if (return_date) {
                ...
                logger_1.logger.info(`Return date filter: ${return_date}`);
            }
            if (seat_class) {
                where.seat_class = {
                    equals: seat_class,
            ...
            if (direct_flights_only) {
                ...
                logger_1.logger.info("Filtering for direct flights only");
            }
            // Get total count (without filters)
            const totalCount = await prisma_1.prisma.flight.count();
            // Get filtered count
            const filteredCount = await prisma_1.prisma.flight.count({ where });
            // Get paginated data
            ...
            logger_1.logger.error(`Error in getFlights: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get flights", 500);
        }
    }
    static async getFlightById(id) {
        try {
            ...
            logger_1.logger.error(`Error in getFlightById: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to get flight", 500);
        }
    }
    static async updateFlight(id, data) {
        try {
            ...
            const updatedFlight = await prisma_1.prisma.flight.update({
                where: { id },
                data: {
                    flight_number: data.flight_number,
                    airline: data.airline,
            ...
            logger_1.logger.error(`Error in updateFlight: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to update flight", 500);
        }
    }
    static async deleteFlight(id) {
        try {
            ...
            if (activeBookings > 0) {
                throw new appError_1.AppError("Cannot delete flight with active bookings", 400);
            }
            ...
            logger_1.logger.error(`Error in deleteFlight: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to delete flight", 500);
        }
    }
    static async searchFlights(params) {
        try {
            ...
            logger_1.logger.error(`Error in searchFlights: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to search flights", 500);
        }
    }
    static async getFlightAvailability(params) {
        try {
            ...
            logger_1.logger.error(`Error in getFlightAvailability: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to check flight availability", 500);
        }
    }
    // Custom query method for advanced flight searches
    static async customFlightQuery(params) {
        try {
            const { page = 1, limit = 10, sortBy = "departure_time", sortOrder = "asc", 
            // Custom query parameters
            layover_max_hours, preferred_airlines, exclude_airlines, departure_time_start, departure_time_end, arrival_time_start, arrival_time_end, price_range, duration_max_minutes, ...rest } = params;
            // Calculate pagination values
            const skip = (page - 1) * limit;
            ...
            return {
                data: processedFlights,
                metadata: {
                    totalCount: await prisma_1.prisma.flight.count(),
            ...
            logger_1.logger.error(`Error in customFlightQuery: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
            throw new appError_1.AppError("Failed to execute custom flight query", 500);
        }
    }
}
