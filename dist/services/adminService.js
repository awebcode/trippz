...
class AdminService {
    ...
    static async getUsers(query) {
        try {
            ...
            const filteredCount = await prisma_1.prisma.user.count({ where });
            ...
            throw new appError_1.AppError("Failed to get users", 500);
        }
    }
    static async getUserById(id) {
        try {
            ...
            throw new appError_1.AppError("Failed to get user", 500);
        }
    }
    static async updateUserRole(id, role) {
        try {
            ...
            logger_1.logger.error(`Error in updateUserRole: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
    ...
    static async getStats() {
        try {
            const [totalUsers, totalServiceProviders, totalTravelAgencies, totalDestinations, totalPackages, totalServices, totalServiceOrders, totalPackageOrders, totalBookings, totalRevenue,] = await Promise.all([
                prisma_1.prisma.user.count(),
                prisma_1.prisma.serviceProvider.count(),
                prisma_1.prisma.travelAgency.count(),
                prisma_1.prisma.destination.count(),
                prisma_1.prisma.travelPackage.count(),
                prisma_1.prisma.service.count(),
                prisma_1.prisma.serviceOrder.count(),
                prisma_1.prisma.packageOrder.count(),
            ...
            throw new appError_1.AppError("Failed to get stats", 500);
        }
    }
    static async getAnalytics(query) {
        try {
            ...
            const serviceOrdersByStatus = await prisma_1.prisma.serviceOrder.groupBy({
                by: ["status"],
                _count: {
                    id: true,
                },
            });
            const packageOrdersByStatus = await prisma_1.prisma.packageOrder.groupBy({
                by: ["status"],
                _count: {
                    id: true,
                },
            ...
            const topDestinations = await prisma_1.prisma.destination.findMany({
                take: 10,
                include: {
                    packages: {
                        include: {
            ...
            throw new appError_1.AppError("Failed to get analytics", 500);
        }
    }
    // System Settings
    static async getSystemSettings() {
        try {
            ...
            if (!settings) {
                ...
                return await prisma_1.prisma.systemSetting.create({
                    data: {
            ...
            throw new appError_1.AppError("Failed to get system settings", 500);
        }
    }
    static async updateSystemSettings(data) {
        try {
            ...
            return await prisma_1.prisma.systemSetting.update({
            ...
            throw new appError_1.AppError("Failed to update system settings", 500);
        }
    }
    // Content Management
    static async getAllDestinations(query) {
        try {
            ...
            // Execute count query for total records
            const totalCount = await prisma_1.prisma.destination.count();
            // Execute count query with filters
            const filteredCount = await prisma_1.prisma.destination.count({ where });
            // Execute main query with pagination, sorting, and filtering
            ...
            logger_1.logger.error(`Error in getAllDestinations: ${error}`);
            if (error instanceof appError_1.AppError) {
                throw error;
            }
    ...
    static async getUserRegistrationsByPeriod(period, dateFrom, dateTo) {
        let dateFormat;
        let groupBy;
        switch (period) {
            case "daily":
        ...
        const registrations = await prisma_1.prisma.$queryRaw `
      SELECT 
    ...
    static async getBookingsByPeriod(period, dateFrom, dateTo) {
        let dateFormat;
        let groupBy;
        switch (period) {
            case "daily":
        ...
        const bookings = await prisma_1.prisma.$queryRaw `
      SELECT 
        TO_CHAR(created_at, ${dateFormat}) as period,
        COUNT(*) as count,
    ...
    static async getRevenueByPeriod(period, dateFrom, dateTo) {
        let dateFormat;
        let groupBy;
        switch (period) {
            case "daily":
